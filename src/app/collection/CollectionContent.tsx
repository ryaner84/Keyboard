...
export default function CollectionContent() {
  ...
  const watching = useMemo(
    () => items.filter((item) => !item.collection.inCollection),
    [items]
  );
  ...
  async function shareCollection() {
    ...
    if (publicItems.length === 0) {
      setNotice("Choose at least one owned item to display publicly before sharing.");
      setTab("collection");
      return;
    }
    ...
    try {
      ...
      setNotice("Public collection link copied. Only your selected public items are visible.");
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Could not share the collection");
    }
  }
      ...
      <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
        ...
        {authenticated && (
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-black/5 bg-white/70 px-4 py-3 text-xs text-gray-500 backdrop-blur dark:border-white/10 dark:bg-white/5 dark:text-gray-400">
            ...
            <span>
              {profile?.collectionPublished ? "Public page published" : "Public page not published"}
            </span>
          </div>
        )}
        ...
        <CollectionItemEditor
          ...
          onMoveToTracking={async () => {
            try {
              ...
              setEditingItem(null);
              setTab("tracking");
              setNotice("Item moved back to tracking.");
            } catch (error) {
